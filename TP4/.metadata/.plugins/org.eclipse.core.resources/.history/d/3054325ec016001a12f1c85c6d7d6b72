...
{
	for (vector<Vehicule*>::iterator iter = m_vVehicules.begin(); iter != m_vVehicules.end(); iter++)
	{
 		delete *iter;
	}
}
...
/**
 * \brief Méthode qui permet de vérifier si le Propriétaire a déjà un véhicule.
 * \param[in] p_niv est le niv du véhicule à vérifier
 * \return un booléen, vrai si le véhicule testé est déjà possédé par le propriétaire
 */
...
/**
 * \brief Teste l'invariant de la classe Proprietaire. L'invariant de cette classe s'assure que les informations du propriétaire sont valides
 */
